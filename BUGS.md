...
| **K12** | Dashboard ↔ Reports | Same metric returns different numbers on dashboard widget vs Reports module for the same date range | **Critical** | TESTPLAN.md only — Reports module out of automation scope |
...
| **K14** | Settings › Subscription | Per-clinic subscription hyperlink (e.g. "Teal Swing Sandbox") routes to the Templates page instead of subscription detail | Major | TESTPLAN.md only — Settings/Subscription out of scope |
| **K15** | Settings › Subscription | Default payment-method card is deletable with no warning, leaving the account without a card on file | **Critical** | TESTPLAN.md only — Settings/Subscription out of scope |
| **K16** | Auth › Login | Error message for invalid credentials renders inconsistently (sometimes inline plain text, sometimes attached to a form label) | Minor | TESTPLAN.md only — Auth has no per-feature plan yet |
| **K17** | Auth › Login | Lockout / rate-limit timer behaves inconsistently — wait time varies between attempts even when the trigger looks identical | Major | TESTPLAN.md only — Auth has no per-feature plan yet |
