...
| **K12** | Dashboard ↔ Reports | Same metric returns different numbers on dashboard widget vs Reports module for the same date range. **UNVERIFIED — flagged 2026-05-13, needs re-verify against v4.** | **Critical** | TESTPLAN.md only — Reports module out of automation scope |
...
| **K14** | Settings › Subscription | Per-clinic subscription hyperlink (e.g. "Teal Swing Sandbox") routes to the Templates page instead of subscription detail. **UNVERIFIED on v4** — Settings/Subscription URL paths probed 2026-05-13 returned 404 on `/admin/clinics/{id}/settings*`; needs UI-driven re-verify. | Major | TESTPLAN.md only — Settings/Subscription out of scope |
| **K15** | Settings › Subscription | Default payment-method card is deletable with no warning, leaving the account without a card on file. **UNVERIFIED on v4** — same access blocker as K14. | **Critical** | TESTPLAN.md only — Settings/Subscription out of scope |
...
| **K22** | Patients › Search | A patient with a non-ASCII firstName (e.g. `Müller<marker>` + lastName `张伟`) saves correctly and is visible on `/admin/clinics/{id}/patients/{id}/edit`, but is **invisible to the list search** even when the query is an ASCII substring of the firstName. The same search pattern works for fully-ASCII patients in the same suite. Verified 2026-05-11 on v3, clinic 1080. | Major | `tests/patients/patients-boundary.spec.ts` › "save accepts unicode names (accented + CJK)" |
| **K23** | Auth › Login | `POST /admin/login_check` returns **HTTP 500 ("Oops! An Error Occurred")** when invalid credentials are submitted, instead of redirecting back to the login form with an inline error. Probe-verified 2026-05-13 on v4 with `invalid_user@example.com` / `WrongPassword123!` — response body is the Symfony 500 error template, not an auth-error page. User has no visible feedback path other than the raw 500 page. | **Critical** | `tests/auth/login-known-bugs.spec.ts` › "[K23] invalid credentials should land on the login form with an error, not a 500" (currently `test.fail()` — flips to red when fixed) |
...
**K23 (Critical).** Replaces the previously-tracked K16 (which described a milder "inconsistent inline error" rendering). The current v4 behavior is more severe: a bad-credentials submit hits the Symfony 500 page directly. Users get no actionable feedback and may assume the site is down rather than realizing the credentials are wrong. Promote the probe spec to a tripwire (`tests/auth/login-known-bugs.spec.ts`) once we agree on the expected post-fix behavior.

## Dropped (no longer reproducible)

- **K16 (was Minor, Auth › Login)** — "Invalid-credentials error renders inconsistently." Dropped 2026-05-13: probe against v4 shows the current behavior is a 500 page (now tracked as **K23**, not the documented "inline plain text / form label" inconsistency). Original v3 observation could not be reproduced.
