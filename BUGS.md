...
- **K23 (was Critical, Auth › Login)** — "POST /admin/login_check returns HTTP 500 on invalid credentials." Dropped 2026-05-13: no longer reproducing on either v3 or v4 via the non-existent-username path. An automated probe at ~07:00 UTC against v4 returned the Symfony 500 page; a manual re-check by Piyush a few hours later on both envs returned the correct inline error *"This account does not exist. Please enter valid credentials..."*. Either v4 was in a transient bad state, or the 500 path requires a different input class (e.g. real user + wrong password) that we haven't manually verified yet. The K23 tripwire spec (`tests/auth/login-known-bugs.spec.ts`) is now a positive regression test that locks in the inline-error contract — if the 500 reappears, the test goes red.
